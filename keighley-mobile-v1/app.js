...
function ring(r,t,w,material=M.rubber){const mesh=new THREE.Mesh(new THREE.TorusGeometry(r,t,8,mobile?28:40),material);mesh.rotation.y=Math.PI/2;mesh.scale.x=w;mesh.castShadow=true;return mesh}
...
async function init(){
 ...
 try{renderer=new THREE.WebGLRenderer({antialias:!mobile,alpha:false,powerPreference:'high-performance',failIfMajorPerformanceCaveat:false})}catch(error){fallback.style.display='grid';throw error};renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.25;renderer.shadowMap.enabled=!mobile;renderer.shadowMap.type=THREE.PCFSoftShadowMap;stage.prepend(renderer.domElement);resize();
 scene.add(new THREE.HemisphereLight(0xb8c8ff,0x130d08,2.2));const key=new THREE.DirectionalLight(0xffd7a8,4.2);key.position.set(-8,11,9);key.castShadow=!mobile;key.shadow.mapSize.set(1024,1024);scene.add(key);const rim=new THREE.PointLight(0xe8a33d,42,30,1.5);rim.position.set(7,5,-7);scene.add(rim);const fill=new THREE.PointLight(0x8ba8ff,24,35,1.6);fill.position.set(10,2,8);scene.add(fill);
