import * as THREE from 'https://rawcdn.githack.com/mrdoob/three.js/d3b629c0c2097cec664ad16369bb6eae3b10e335/build/three.module.js';
...
let scene,camera,renderer,clock,rig,blimp,periscope,odyssey,active=0,selected='body',rolling=false,pan=0,tilt=0,drag=false,lastX=0,lastY=0,audioCtx,transportGain,lastFrame=0;
const parts={},home={};

function mat(color,metalness=.2,roughness=.55){return new THREE.MeshStandardMaterial({color,metalness,roughness})}
const M={carbon:mat(0x25292c,.24,.42),metal:mat(0x8a8f91,.9,.24),black:mat(0x151719,.58,.36),rubber:mat(0x070707,.03,.92),glass:new THREE.MeshStandardMaterial({color:0x8fb7ff,transparent:true,opacity:.58,roughness:.18,metalness:.05,depthWrite:false}),amber:new THREE.MeshBasicMaterial({color:0xe8a33d}),red:new THREE.MeshBasicMaterial({color:0xff3b30})};
function box(w,h,d,material=M.carbon,r=.08){const g=new THREE.BoxGeometry(w,h,d,1,1,1);const mesh=new THREE.Mesh(g,material);mesh.castShadow=mesh.receiveShadow=!mobile;return mesh}
function cyl(r,len,material=M.black,segments=32){const mesh=new THREE.Mesh(new THREE.CylinderGeometry(r,r,len,mobile?Math.min(segments,24):segments),material);mesh.rotation.z=Math.PI/2;mesh.castShadow=mesh.receiveShadow=!mobile;return mesh}
function ring(r,t,w,material=M.rubber){const mesh=new THREE.Mesh(new THREE.TorusGeometry(r,t,mobile?6:8,mobile?24:40),material);mesh.rotation.y=Math.PI/2;mesh.scale.x=w;mesh.castShadow=!mobile;return mesh}
...
function makeBlimp(){const g=new THREE.Group(),m=new THREE.MeshStandardMaterial({color:0x151515,metalness:.25,roughness:.7});const panels=[[8.8,.3,4.7,0,2.25,0],[8.8,.3,4.7,0,-2.25,0],[.3,4.2,4.7,4.25,0,0],[.3,4.2,4.7,-4.25,0,0],[8.2,4.2,.3,0,0,-2.2]];for(const [w,h,d,x,y,z] of panels){const p=box(w,h,d,m);p.position.set(x,y,z);g.add(p)}g.visible=false;return g}
...
function setActive(index){active=index;$$('.section').forEach((s,i)=>s.classList.toggle('active',i===index));$('#anatomy-ui').classList.toggle('show',index===2);$('#operator-ui').classList.toggle('show',index===6);stage.style.touchAction='pan-y';if(index!==6){drag=false;$('#letterbox').classList.remove('on')}}
...
async function init(){
 ...
 try{renderer=new THREE.WebGLRenderer({antialias:!mobile,alpha:false,powerPreference:'high-performance',failIfMajorPerformanceCaveat:false})}catch(error){fallback.style.display='grid';throw error}renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=mobile?1.48:1.3;renderer.shadowMap.enabled=!mobile;renderer.shadowMap.type=THREE.PCFSoftShadowMap;stage.prepend(renderer.domElement);resize();
 scene.add(new THREE.HemisphereLight(0xc7d4ff,0x24170e,mobile?2.8:2.3));const key=new THREE.DirectionalLight(0xffd7a8,mobile?5.2:4.4);key.position.set(-8,11,9);key.castShadow=!mobile;key.shadow.mapSize.set(1024,1024);scene.add(key);const rim=new THREE.PointLight(0xe8a33d,42,30,1.5);rim.position.set(7,5,-7);scene.add(rim);const fill=new THREE.PointLight(0x8ba8ff,24,35,1.6);fill.position.set(10,2,8);scene.add(fill);
 const floor=new THREE.Mesh(new THREE.PlaneGeometry(80,80),mat(0x070707,.25,.46));floor.rotation.x=-Math.PI/2;floor.position.y=-2.1;floor.receiveShadow=!mobile;scene.add(floor);
...
function loop(now){requestAnimationFrame(loop);if(!renderer||document.hidden)return;if(mobile&&now-lastFrame<33)return;lastFrame=now;const t=clock.getElapsedTime();if(active!==6){const baseFov=mobile?48:38;if(camera.fov!==baseFov){camera.fov=baseFov;camera.updateProjectionMatrix()}}resetParts();rig.visible=active<7&&active!==6;odyssey.visible=active>=6;blimp.visible=active===3;periscope.visible=active===4;
 ...
 renderer.render(scene,camera);if(!stage.classList.contains('ready'))stage.classList.add('ready')
}
...
$('#focus').addEventListener('input',()=>{});$('#tilt').addEventListener('input',e=>{$('#tilt-out').textContent=+e.target.value>18?'Up':+e.target.value<-18?'Down':'Level'});$('#imax').addEventListener('click',()=>{$('#imax').classList.add('active');$('#standard').classList.remove('active');$('#letterbox').classList.remove('on')});$('#standard').addEventListener('click',()=>{$('#standard').classList.add('active');$('#imax').classList.remove('active');$('#letterbox').classList.add('on')});$('#roll').addEventListener('click',()=>setRolling(!rolling));
stage.addEventListener('pointerdown',e=>{if(active!==6)return;drag=true;lastX=e.clientX;stage.setPointerCapture?.(e.pointerId)});stage.addEventListener('pointermove',e=>{if(!drag||active!==6)return;pan=clamp(pan+(e.clientX-lastX)/innerWidth*1.8,-1,1);lastX=e.clientX});stage.addEventListener('pointerup',e=>{drag=false;stage.releasePointerCapture?.(e.pointerId)});stage.addEventListener('pointercancel',()=>drag=false);
document.addEventListener('visibilitychange',()=>{if(document.hidden&&rolling)setRolling(false)});
init().catch(error=>{console.error(error);status.textContent='2D fallback · renderer unavailable';fallback.style.display='none'});
